...
interface IIBOv3Controller {
    event NewIBOCreated(address indexed pool, address creator);
    ...
    function getMaxAllocationAmountAtTierId(uint256 tierId, uint256 baseAmount) external view returns (uint256);
}


pragma solidity ^0.8.0;
contract IBO is Ownable, ReentrancyGuard {
    ...
    IIBOv3Controller public controller;
    ...
    mapping(address => User) public tierFunders;
    ...
    event IBOTokenInfoChanged(uint256 saleTarget, uint256 fundTarget);
    ...
    event IBOProgressChanged(address buyer, uint256 amount, uint256 fundRaised, uint256 saleRaised, uint256 roundId);

    event IBOClaimed(address to, uint256 amount);
    ...
    modifier canRaise(address addr, uint256 amount) {
        ...
        if (currentRoundId <= 4) {
            if (whitelistEnabled) {
                require(isTierWhitelisted[addr], "Not whitelisted");
            }
        } else {
            require(fundRaised + amount <= fundTarget, "Target hit!");
        }

        uint256 personalTotal = amount + tierFunders[addr].totalFunded;
    ...
    function setBaseData(
        ...
        emit IBOBaseDataChanged(startTime, endTime, claimTime, minFundAmount, baseAmount, meta);
    }

    function setWhitelistEnabled(bool _whitelistEnabled) external isOperatorOrOwner {
    ...
    function getCurrentRoundId() public view returns (uint256) {
        ...
        // 1,2,3,4: tier round, 5: white, 6: fcfs
    ...
    function getRoundTotalAllocation(uint256 currentRoundId) public view returns (uint256) {
        ...
        if (currentRoundId <= 4) {
            // tiers
            return (fundTarget * roundsPercents[currentRoundId - 1]) / allocationMultiplier;
        }
        return fundTarget + roundsFundRaised[currentRoundId] - fundRaised;
    }
    ...
    function getMaxAllocation(address addr) public view returns (uint256) {
        ...
        if (currentRoundId <= 4) {
            ...
            if (controller.getTierId(addr) == 0) {
                return 0;
            } else {
        ...
        // iron allocation
        return controller.getMaxAllocationAmountAtTierId(1, baseAmount);
    }
    ...
    function getFunderInfo(address funder) external view returns (User memory) {
        ...
        info.totalFunded =
            tierFunders[funder].totalFunded +
            whitelistFunders[funder].totalFunded +
            fcfsFunders[funder].totalFunded;

        info.released = tierFunders[funder].released + whitelistFunders[funder].released + fcfsFunders[funder].released;

        return info;
    }

    function setTierWhitelist(address[] calldata addrs, bool _whitelist) external isOperatorOrOwner {
        for (uint256 index = 0; index < addrs.length; index++) {
            isTierWhitelisted[addrs[index]] = _whitelist;
        }
    }

    function setRoundsAllocationPercents(uint256[] calldata allocations, uint256 _allocationMultiplier)
        external
        isOperatorOrOwner
    {
        require(allocations.length == 4, "Invalid allocs");
        uint256 total = 0;
        for (uint256 index = 0; index < 4; index++) {
            total = total + allocations[index];
            roundsPercents[index] = allocations[index];
        }

        allocationMultiplier = _allocationMultiplier;

        require(total <= allocationMultiplier, "Too big");
    }
    ...
    function setSaleTarget(uint256 _saleTarget) external isOperatorOrOwner {
        ...
        saleTarget = _saleTarget;
        emit IBOTokenInfoChanged(saleTarget, fundTarget);
    }
    ...
    function withdrawRemainingSaleToken() external isOperatorOrOwner {
        require(block.timestamp > endTime, "IBO has not yet ended");
    ...
    function getTierClaimableAmount(address addr) private view returns (uint256) {
        uint256 maxAllocation = controller.getMaxAllocationAmount(addr, baseAmount);
        if (maxAllocation < tierFunders[addr].totalFunded) {
            return 0;
        }
        return getUnlockedTokenAmount(addr, tierFunders[addr].totalFunded) - tierFunders[addr].released;
    }
    ...
    function getClaimableAmount(address addr) public view returns (uint256) {
        return getTierClaimableAmount(addr) + getWhitelistClaimableAmount(addr) + getFCFSClaimableAmount(addr);
    }
    ...
    function _claimTo(address to) private {
        ...
        if (claimableAmount > 0) {
            tierFunders[to].released = tierFunders[to].released + getTierClaimableAmount(to);
            ...
            emit IBOClaimed(to, claimableAmount);
        }
    }
    ...
    function _processBuy(address buyer, uint256 amount) private {
        ...
        if (currentRoundId <= 4) {
            if (tierFunders[buyer].totalFunded == 0) {
                funderAddresses.push(buyer);
            }

            tierFunders[buyer].totalFunded = tierFunders[buyer].totalFunded + amount;

            roundId = 5 - controller.getTierId(buyer);

            require(roundsFundRaised[roundId] + amount <= getRoundTotalAllocation(roundId), "Target hit!");
        } else if (currentRoundId == 5) {
            // whitelist
            if (tierFunders[buyer].totalFunded == 0 && whitelistFunders[buyer].totalFunded == 0) {
                funderAddresses.push(buyer);
            }
            ...
            // fcfs
            if (
                tierFunders[buyer].totalFunded == 0 &&
                whitelistFunders[buyer].totalFunded == 0 &&
                fcfsFunders[buyer].totalFunded == 0
            ) {
                funderAddresses.push(buyer);
            }
        ...
        emit IBOProgressChanged(buyer, amount, fundRaised, saleRaised, currentRoundId);
    }
